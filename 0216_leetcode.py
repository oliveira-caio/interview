...
problem: Find all valid combinations of k numbers that sum up to n such that the
...
class Solution:
    def combinationSum3(self, k: int, n: int) -> List[List[int]]:
        def backtrack(target, start, output, comb):
            if target == 0 and len(comb) == k:
                ...
                for i in range(start, 10):
                    comb.append(i)
                    backtrack(target - i, i + 1, output, comb)
                    comb.pop()

        output = []
        backtrack(n, 1, output, [])
        return output
