...
problem: Design a hit counter which counts the number of hits received in the
...
class HitCounter:
    ...
    def hit(self, timestamp: int) -> None:
        self.hits[timestamp] = 1 + self.hits.get(timestamp, 0)

    def getHits(self, timestamp: int) -> int:
        return sum((v for k, v in self.hits.items() if k > timestamp - 300))
