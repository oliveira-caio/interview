...
problem: The next greater element of some element x in an array is the first
    ...
    def nextGreaterElement(self, nums1, nums2):
        ...
        for i in range(len(nums2) - 1, -1, -1):
            while stack and nums2[i] > nums2[stack[-1]]:
                stack.pop()
            max_right[nums2[i]] = -1 if not stack else nums2[stack[-1]]
            stack.append(i)

        for n in nums1:
            output.append(max_right[n])
