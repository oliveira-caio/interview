...
problem: The boundary of a binary tree is the concatenation of the root, the
left boundary, the leaves ordered from left-to-right, and the reverse order of
the right boundary.
    ...
    def boundaryOfBinaryTree(self, root: Optional[TreeNode]) -> List[int]:
        ...
        def leaves(node):
            ...
            if node != root and not node.left and not node.right:
                return [node.val]
            leaves_left = leaves(node.left)
            leaves_right = leaves(node.right)
            return leaves_left + leaves_right

        return ([root.val] + left_boundary(root.left)
                + leaves(root) + right_boundary(root.right))
