...
problem: On a single-threaded CPU, we execute a program containing n functions.
...
Example 2:
Input: n = 1, logs = ["0:start:0", "0:start:2", "0:end:5", "0:start:6",
"0:end:6", "0:end:7"]
Output: [8]
Explanation:
...
Example 3:
Input: n = 2, logs = ["0:start:0", "0:start:2", "0:end:5", "1:start:6",
"1:end:6", "0:end:7"]
Output: [7,1]
Explanation:
- Function 0 starts at the beginning of time 0, executes for 2 units of time,
...
1spends 1 unit of total time executing.
...
class Solution:
    def exclusiveTime(self, n: int, logs: List[str]) -> List[int]:
        ...
        for log in logs:
            function, status, time = tuple(log.split(':'))
            ...
            if status == 'start':
                ...
                last_function, last_time = stack.pop()
                result[function] += time - last_time + 1
                if stack:
                    stack[-1][1] = time + 1
