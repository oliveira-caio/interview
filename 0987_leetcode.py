...
problem: Given the root of a binary tree, calculate the vertical order traversal
of the binary tree.
...
class Solution:
    def verticalTraversal(self, root: Optional[TreeNode]) -> List[List[int]]:
        ...
        stack = [(0, 0, root)]
        while stack:
            col, row, curr = stack.pop()
            nodes.append((col, row, curr.val))
            if curr.left:
                stack.append((col - 1, row + 1, curr.left))
            if curr.right:
                stack.append((col + 1, row + 1, curr.right))
        nodes.sort()
        res = {}
