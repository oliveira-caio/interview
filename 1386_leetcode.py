...
problem: A cinema has n rows of seats, numbered from 1 to n and there are ten
...
across an aisle (such as [3,3] and [3,4]) are not considered to be adjacent, but
there is an exceptional case on which an aisle split a four-person group, in
...
class Solution:
    def maxNumberOfFamilies(self, n: int, reservedSeats: List[List[int]]) -> int:
        ...
        for row in seats:
            n_families += -2 if len(seats[row]) == 3 else -1
