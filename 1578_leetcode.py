...
problem: Alice has n balloons arranged on a rope. You are given a 0-indexed
...
class Solution:
    def minCost(self, colors: str, neededTime: List[int]) -> int:
        left, right, time = 0, 1, 0

        while right < len(colors):
            if colors[left] == colors[right]:
                max_color = (left, neededTime[left])
                while right < len(colors) and colors[right] == colors[left]:
                    if neededTime[right] > neededTime[max_color[0]]:
                        max_color = (right, neededTime[right])
                    right += 1
