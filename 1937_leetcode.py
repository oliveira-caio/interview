...
problem: You are given an m x n integer matrix points (0-indexed). Starting with
0 points, you want to maximize the number of points you can get from the matrix.
...
class Solution:
    def maxPoints(self, points: List[List[int]]) -> int:
        def left_dp(curr_row):
            res = [curr_row[0]] + [0]*(len(curr_row) - 1)
            for i in range(1, len(curr_row)):
                res[i] = max(res[i - 1] - 1, curr_row[i])
            return res

        def right_dp(curr_row):
            res = [0]*(len(curr_row) - 1) + [curr_row[-1]]
            for i in range(2, 1 + len(curr_row)):
                res[-i] = max(res[-i + 1] - 1, curr_row[-i])
            return res
        ...
        for i in range(1, len(points)):
            left, right, temp = left_dp(res), right_dp(res), res
